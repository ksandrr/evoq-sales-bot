...
4. Manual Telegram smoke checks were not run in this session:
   - text parsing flow
   - voice flow with `РќР°Р·РІР°РЅРёРµ`, `РћРїРёСЃР°РЅРёРµ`, `РўСЂР°РЅСЃРєСЂРёРїС†РёСЏ`
   - Weeek task creation
...
3. Run one live Telegram smoke test for voice and confirm the `РќР°Р·РІР°РЅРёРµ` / `РћРїРёСЃР°РЅРёРµ` / `РўСЂР°РЅСЃРєСЂРёРїС†РёСЏ` block.
...
- Filtered Weeek column selection so `Р“РѕС‚РѕРІРѕ` is not shown as a selectable target column for creating a new task.
...
3. First manual checks to run:
   - voice from empty chat: `РњРёСЂР°, Р·Р°РїРёС€Рё Р·Р°РґР°С‡Сѓ РІ Р’РРљ РІ Р»РёС‡РЅРѕРµ РєСѓРїРёС‚СЊ РјРѕР»РѕРєР° Р·Р°РІС‚СЂР° РІ 22:00`
   - voice while another draft is active: must override into a new Weeek flow
   - manual button flow through `Р—Р°РґР°С‡Р° Р’РРљ`
   - confirm `Р“РѕС‚РѕРІРѕ` is no longer offered in the column picker
...
- Remove the old main-menu buttons for ideas, local tasks, reminders, test, and manual `Р—Р°РґР°С‡Р° Р’РРљ`.
- Make plain phrases like `РњРёСЂР°, РїСЂРёРІРµС‚, Р·Р°РїРёС€Рё Р·Р°РґР°С‡Сѓ ...` route straight into the Weeek/VIK task flow by default.
- Add a single main-menu button to browse VIK tasks by project and see their statuses.

### Code Changes

- In `bot.py`:
  - added `BTN_LIST_WEEEK_TASKS = "рџ“‚ Р—Р°РґР°С‡Рё РІ Р’РРљ"`
  - simplified `main_menu_keyboard()` to only show `рџ“‚ Р—Р°РґР°С‡Рё РІ Р’РРљ` and `в„№пёЏ РџРѕРјРѕС‰СЊ`
  ...
  - wired the new main-menu message handler for `рџ“‚ Р—Р°РґР°С‡Рё РІ Р’РРљ`
- In `tests/test_time_parsing.py`:
...
1. The new `рџ“‚ Р—Р°РґР°С‡Рё РІ Р’РРљ` screen assumes Weeek task list items either contain a readable status name in task payload or can be resolved through project boards/columns.
...
2. Verify only `рџ“‚ Р—Р°РґР°С‡Рё Р’РРљ` is present in the keyboard.
...
- Server validation after deploy:
  - `.venv/bin/python -m py_compile bot.py database.py weeek_client.py tests/test_time_parsing.py` вЂ” passed
  - `.venv/bin/python -m unittest tests.test_time_parsing` вЂ” passed (`47 tests`, `OK`)
- Live service status after restart:
  - `mira-task-bot.service` is `active (running)`
  - `Main PID`: `63554`
